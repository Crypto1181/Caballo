...
class CryptoDetailScreen extends StatefulWidget {
  ...
  const CryptoDetailScreen({super.key, required this.coinId, required this.symbolLabel, required this.displayName});
...
class _CryptoDetailScreenState extends State<CryptoDetailScreen> {
  ...
  String _rapidApiKey = 'cfd42cb60fmsh596e3adef1f26a9p16f12bjsn720e2168c172';
  String _rapidApiHost = 'trade-bloom-real-time.p.rapidapi.com';
...
<head>
  ...
  <style>html,body{height:100%;margin:0;background:transparent;}#chart{position:absolute;left:0;top:0;right:0;bottom:0;}</style>
...
<body>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.bolt_outlined, color: isDark ? Colors.white : Colors.black, size: 24),
            onPressed: () {},
          ),
          IconButton(
            icon: Icon(Icons.star_outline, color: isDark ? Colors.white : Colors.black, size: 24),
            onPressed: () {},
          ),
          IconButton(
            icon: Icon(Icons.share_outlined, color: isDark ? Colors.white : Colors.black, size: 24),
            onPressed: () {},
          ),
        ],
      ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                              _currentPrice > 0 ? '\$${_currentPrice.toStringAsFixed(2)}' : 'Loading...',
                              style: TextStyle(
                                fontSize: 36,
                                fontWeight: FontWeight.bold,
                                color: isDark ? Colors.white : Colors.black,
                            ...
                            Row(
                              children: [
                                Icon(
                                  _priceChange >= 0 ? Icons.arrow_upward : Icons.arrow_downward,
                                  size: 16,
                                  color: _priceChange >= 0 ? Colors.green : Colors.red,
                                ),
                                const SizedBox(width: 4),
                                Text(
                                  _currentPrice > 0 ? '\$${_priceChange.abs().toStringAsFixed(2)} (${_priceChangePercent.abs().toStringAsFixed(2)}%)' : '',
                                  style: TextStyle(
                                    fontSize: 16,
                ...
                children: [
                  InAppWebView(
                    ...
                    initialOptions: InAppWebViewGroupOptions(
                      crossPlatform: InAppWebViewOptions(javaScriptEnabled: true, transparentBackground: true),
                    ),
                  ...
                  if (_error != null)
                    Center(
                      child: Padding(
                        ...
                        child: Text(_error!, textAlign: TextAlign.center, style: TextStyle(color: isDark ? Colors.white : Colors.black)),
                      ),
                    ),
                ],
              ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  _buildTransactionItem('Sold BTC', 'Sep 20, 2025', -107.06, -0.000932, isDark),
                ],
              ),
            ),
            const SizedBox(height: 24),
    ...
    return InkWell(
      ...
      child: Text(
        ...
        style: TextStyle(
          ...
          color: selected ? Colors.blue : (isDark ? Colors.grey[400] : Colors.grey[600]),
        ),
      ),
    );
  }
  ...
  Widget _buildTabContent(bool isDark) {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('\$0.00', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black)),
                Text('0 BTC', style: TextStyle(fontSize: 14, color: isDark ? Colors.grey[400] : Colors.grey[600])),
              ],
            ),
          ),
        ],
    ...
    return Text('Insights coming soon...', style: TextStyle(color: isDark ? Colors.grey[400] : Colors.grey[600]));
  }

  Widget _buildTransactionItem(String title, String date, double usd, double btc, bool isDark) {
    ...
    return Row(
      children: [
        Container(
          ...
          height: 40,
          decoration: BoxDecoration(color: Colors.orange, shape: BoxShape.circle),
          child: const Center(child: Text('₿', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white))),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(date, style: TextStyle(fontSize: 14, color: isDark ? Colors.grey[400] : Colors.grey[600])),
            ],
          ),
        ),
        Column(
          ...
          children: [
            Text(isNegative ? '\$${usd.abs().toStringAsFixed(2)}' : '\$${usd.toStringAsFixed(2)}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.red)),
            Text(isNegative ? '-${btc.abs()} BTC' : '${btc} BTC', style: TextStyle(fontSize: 14, color: Colors.red)),
          ],
        ),
      ],
    );
  ...
  Widget _buildTfButton(String tf, bool isDark) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? Colors.blue : (isDark ? Colors.grey[900] : Colors.grey[100]),
  ...
  Widget _buildTypeToggle(bool isDark) {
    Widget pill(String t, String label) {
      final selected = _seriesType == t;
      return InkWell(
        onTap: () {
          if (_seriesType == t) return;
          setState(() { _seriesType = t; });
          final seriesOptions = {
            'upColor': '#26a69a',
            'downColor': '#ef5350',
            'wickUpColor': '#26a69a',
          ...
          _controller?.evaluateJavascript(source: "setSeriesType('${t}', ${jsonEncode(seriesOptions)});");
        },
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
          decoration: BoxDecoration(
          ...
          child: Text(label, style: TextStyle(fontSize: 12, color: selected ? Colors.white : (isDark ? Colors.grey[400] : Colors.grey[700]), fontWeight: selected ? FontWeight.bold : FontWeight.w500)),
        ),
      );
    }
    return Row(children: [pill('candlestick','C'), const SizedBox(width: 6), pill('line','L'), const SizedBox(width: 6), pill('area','A')]);
  }
  ...
  Future<void> _fetchPrice() async {
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final currentPrice = double.tryParse(map['lastPrice']?.toString() ?? '') ?? 0.0;
        final openPrice = double.tryParse(map['openPrice']?.toString() ?? '') ?? 0.0;
      ...
      try {
        final uri = Uri.parse('https://api.coingecko.com/api/v3/simple/price?ids=${widget.coinId}&vs_currencies=usd&include_24hr_change=true');
        final resp = await http.get(uri);
        if (resp.statusCode == 200) {
          final map = jsonDecode(resp.body) as Map<String, dynamic>;
          final coinData = map[widget.coinId] as Map<String, dynamic>?;
          if (coinData != null) {
            final price = double.tryParse(coinData['usd']?.toString() ?? '') ?? 0.0;
            final changePercent = double.tryParse(coinData['usd_24h_change']?.toString() ?? '') ?? 0.0;
            if (price > 0 && mounted) {
              setState(() {
                _currentPrice = price;
  ...
  Future<void> _waitForInit() async {
    for (int i = 0; i < 40; i++) {
      try {
        final res = await _controller?.evaluateJavascript(source: 'typeof initChart === "function"');
  ...
  String _getBinanceInterval(String tf) {
    switch (tf.toUpperCase()) {
      ...
      case '1W': return '1w';
      case '1M': return '1M';
      case '1Y': return '1M'; // Binance doesn't have 1Y, use 1M with more data
      case 'ALL': return '1M';
      default: return '1h';
    }
  }
  ...
  int _getDataLimit(String tf) {
    switch (tf.toUpperCase()) {
      ...
      case '1W': return 104; // 2 years of weekly data
      case '1M': return 60; // 5 years of monthly data
      case '1Y': return 120; // 10 years of monthly data
      case 'ALL': return 500; // Max historical data
      default: return 200;
    }
  }
  ...
  Future<void> _injectData(bool isDark) async {
    ...
    try {
      ...
      try {
        ...
        data = await _fetchBinanceKlines(symbol: symbol, interval: interval, limit: limit);
      } catch (binanceError) {
        print('Binance failed, trying CoinGecko: $binanceError');
        // Fallback to CoinGecko
        int days = 30;
        if (_tf == '1H') days = 7;
        else if (_tf == '1D') days = 365;
        else if (_tf == '1W') days = 730;
        else if (_tf == '1M' || _tf == '1Y' || _tf == 'ALL') days = 3650;
      ...
      final volume = data.map((e) => ({
        'time': e['time'],
        'value': e['volume'] ?? 0,
        'color': ((e['close'] as num) >= (e['open'] as num)) ? '#26a69a' : '#ef5350',
      })).toList();

      final script = "initChart(${jsonEncode(data)}, ${jsonEncode(options)}, ${jsonEncode(volume)});";
      ...
      if (mounted) setState(() { _error = 'Failed to load chart data: ${e.toString()}'; _loading = false; });
    }
  }
  ...
  void _startLive() {
    ...
    final url = Uri.parse('wss://stream.binance.com:9443/stream?streams=${symbol}@ticker/${symbol}@kline_$interval');
    ...
    _ws!.stream.listen((event) {
      try {
        ...
        if (stream?.contains('@ticker') ?? false) {
          ...
          _controller?.evaluateJavascript(source: "updatePrice(${jsonEncode({'time': now, 'price': close, 'volume': vol})});");
          ...
          final bar = {'time': (t / 1000).round(), 'open': open, 'high': high, 'low': low, 'close': close, 'isClosed': isClosed};
          final volBar = {'time': (t / 1000).round(), 'value': vol, 'color': close >= open ? '#26a69a' : '#ef5350'};
          _controller?.evaluateJavascript(source: "updateBar(${jsonEncode(bar)}, ${jsonEncode(volBar)});");
        }
      } catch (_) {}
    });
  }
  ...
  Future<List<Map<String, dynamic>>> _fetchCoinGeckoOhlc(String coinId, String vsCurrency, int days) async {
    final uri = Uri.parse('https://api.coingecko.com/api/v3/coins/$coinId/ohlc?vs_currency=$vsCurrency&days=$days');
    final resp = await http.get(uri);
    if (resp.statusCode != 200) {
  ...
  Future<List<Map<String, dynamic>>> _fetchBinanceKlines({required String symbol, required String interval, int limit = 200}) async {
